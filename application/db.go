...
func fetchProformaByCompanyRC(ctx *gin.Context, cid uint, jps *[]JobProforma) error {
	tx := db.WithContext(ctx).Where("company_recruitment_cycle_id = ?", cid).Find(jps)
	return tx.Error
}

func fetchProformaByRC(ctx *gin.Context, rid uint, jps *[]JobProforma) error {
	tx := db.WithContext(ctx).Where("recruitment_cycle_id = ?", rid).Find(jps)
	return tx.Error
}

func fetchJobProforma(ctx *gin.Context, pid uint, jp *JobProforma) error {
	tx := db.WithContext(ctx).Where("id = ?", pid).First(jp)
	return tx.Error
}

func createJobProforma(ctx *gin.Context, jp *JobProforma) error {
	tx := db.WithContext(ctx).Create(jp)
	return tx.Error
}

func updateJobProforma(ctx *gin.Context, jp *JobProforma) error {
	tx := db.WithContext(ctx).Where("id = ?", jp.ID).Updates(jp)
	return tx.Error
}

func deleteJobProforma(ctx *gin.Context, pid uint) error {
